    ...
    <view>
      ...
      <!-- refresher-enabled
        ...
        <view class="default_box" v-if="resList.length === 0">
          ...
          <view class="warn">暂无加钟信息～</view>
        </view>
        <view class="table_header" v-else>
          <view class="little">序号</view>
          <view class="date">日期</view>
          <view>学员</view>
          <view class="date">电话</view>
          <view class="little">车型</view>
          <view class="little">学时</view>
          <view style="flex: 0.8">总价</view>
        </view>
        <view class="table_line" v-for="(item, index) in resList" :key="index">
          <view class="little">{{index + 1}}</view>
          <view class="date">{{item.applyDate.split(' ')[0]}}</view>
          <view>{{item.name}}</view>
          <view class="date click_color" @click="phoneOperate(item.mobile)">{{item.mobile}}</view>
          <view class="little">{{item.carModel}}</view>
          <view class="little">{{item.sumPeriod}}</view>
          <view style="flex: 0.8">{{item.amount}}</view>
        </view>
        <view class="table_line" v-if="resList.length > 0">
          <view class="little">合计</view>
          <view class="date"></view>
          <view></view>
          <view class="date"></view>
          <view class="little"></view>
          <view class="little">{{statistics.sumPeriod}}</view>
          <view style="flex: 0.8">{{statistics.amount}}</view>
        </view>
      </scroll-view>
    </view>
...
export default class Arrange extends Vue {
  ...
  tableHeader = ['序号', '日期', '学员', '电话', '车型', '学时', '总价'];
  ...
  async getData() {
    ...
    this.resList = await this.$http.post('order/v1/scattered/queryClockOrderToApplet', data);
