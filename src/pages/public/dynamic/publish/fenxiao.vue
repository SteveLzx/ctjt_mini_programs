<template>
  <view class="f_wrap">
    <view class="activity_container">
      <view class="title">介绍人页面</view>
      <view class="dec">分享出去直接被分享人(介绍人)打开将看到以下页面</view>
      <view :class="['activity', 'introducer', { disable: downTimes }]">
        <view class="disble_wrap" v-if="downTimes">活动即将在 {{timeName}} 后开始</view>
        <view class="notice" v-if="!downTimes">
          <Notice :data="userData"></Notice>
        </view>
        <ActivityGiftbox :price="formData.bounty" :time="!downTimes ? formData.endDate : ''" :is-show-price="formData.isShow === 1"></ActivityGiftbox>
      </view>
      <ActivityForm :form-data.sync="formData">
        <template #uni-form-item>
          <view class="uni-form-item uni-column">
            <view class="title">奖励金额显示</view>
            <input
              class="uni-input"
              name="input"
              type="number"
              v-model="formData.bounty"
              placeholder="请输入给介绍人的奖励金额"
            />
          </view>
          <view class="uni-form-item uni-column">
            <view class="title">展示奖励金额</view>
            <view>
              <switch name="switch" class="item_switch" @change="handleSwitchIsShow" />
              <label class="item_text">是否在介绍人海报中展示奖励金</label>
            </view>
          </view>
        </template>
      </ActivityForm>
    </view>
    <view class="activity_container recommendee">
      <!-- <view class="title">优惠券设置</view>
      <view class="coupon_settings_c" @click="handleToast">
        <text class="info_t">{{couponData}}</text>
        <text class="iconfont">&#xe6a3;</text>
      </view> -->
      <view class="title">被推荐人页面</view>
      <view class="dec">介绍人再次转发给被推荐人或更多层级转发，打开将看到以下页面</view>
      <view :class="['activity', 'introduced_person', { disable: downTimes }]">
        <view class="disble_wrap" v-if="downTimes">活动即将在 {{timeName}} 后开始</view>
        <view class="notice" v-if="!downTimes">
          <Notice :data="userData"></Notice>
        </view>
...
export default class Fenxiao extends Vue {
  // 海报弹出展示
  showPoster = false;
  ...
  handleSwitchIsShow(e) {
    const { value } = e.detail;
    this.formData.isShow = value ? 1 : 0;
  }
  ...
  @Watch('formData.bounty')
  handleWatchBounty(newVal: number, oldVal: number) {
    if (Number(newVal) > 999) {
      uni.showToast({
        title: '最高奖励金额999',
        icon: 'none'
      });
      setTimeout(() => {
        this.formData.bounty = oldVal;
      }, 0);
    } else {
      this.formData.bounty = Number(newVal);
    }
  }
  ...
  createdActive() {
    ...
    // 校验必填项
      const { beginDate, endDate, bounty } = this.formData;
  ...
  async coachShare() {
    const { name } = this.formData;
    const imageUrl = 'https://file.aicar365.com/mini-program/common/share_card_fx.png';
